...
The archive contains your full WhatsApp message history; we lock it down so other process-users on this machine (e.g. other future service-account archivers) cannot read it. Pattern: dedicated service user owns the runtime dirs, a small read group includes only your login user and the service user.
...
To add another locked-down archiver later (e.g. `_othersvc`), repeat the pattern with `_othersvc` + `othersvc-data` group; your login user joins both read groups but the two service users never join each other's.
