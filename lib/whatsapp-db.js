...
export function searchContacts(db, query, { limit = DEFAULT_LIMIT } = {}) {
  ...
  return db.prepare(`
    ...
    ORDER BY COALESCE(display_name, push_name, id) COLLATE NOCASE
  `).all()
    .map(row => ({ row, score: contactSearchScore(row, normalizedQuery) }))
    .filter(result => Number.isFinite(result.score))
    .sort((a, b) => {
      if (a.score !== b.score) return a.score - b.score
      return compareContactRows(a.row, b.row)
    })
    .slice(0, normalizedLimit)
...
function contactSearchText(row) {
  return [row.display_name, row.push_name, row.contact_name, row.verified_name, row.phone_number, row.lid, row.id]
    .filter(Boolean)
    .map(normalizeContactSearchValue)
    .filter(Boolean)
}

function contactSearchScore(row, query) {
  const values = contactSearchText(row)
  const scores = values.map(value => contactValueSearchScore(value, query))
  return Math.min(...scores)
}

function contactValueSearchScore(value, query) {
  if (value === query) return 0
  if (value.startsWith(query)) return 1
  if (value.includes(query)) return 2

  const distance = boundedEditDistance(value, query, fuzzyDistanceLimit(query))
  if (Number.isFinite(distance)) return 3 + distance

  if (tokenFuzzyMatch(value, query)) return 10
  return Infinity
}

function normalizeContactSearchValue(value) {
  return String(value ?? '')
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
function tokenFuzzyMatch(value, query) {
  const valueTokens = searchTokens(value)
  const queryTokens = searchTokens(query)
  if (!valueTokens.length || !queryTokens.length) return false
  return queryTokens.every(queryToken => valueTokens.some(valueToken => tokenMatches(valueToken, queryToken)))
}

function tokenMatches(valueToken, queryToken) {
  if (valueToken === queryToken || valueToken.startsWith(queryToken) || valueToken.includes(queryToken)) return true
  return Number.isFinite(boundedEditDistance(valueToken, queryToken, fuzzyDistanceLimit(queryToken)))
}
...
function fuzzyDistanceLimit(query) {
  const length = query.replaceAll(' ', '').length
  if (length < 4 || /^\d+$/.test(query)) return 0
...
function boundedEditDistance(a, b, maxDistance) {
  if (maxDistance < 1) return a === b ? 0 : Infinity
